...
foreach ( $attributes['answers'] as $index => $answer ) {
	$answers[] = array( 'text' => $answer, 'index' => $index, 'correct' => ( $index == $attributes['correctAnswer'] ) );
}
$ourContext = array('answers' => $answers, 'solved' => false, 'showCongrats' => false, 'showSorry' => false, 'correctAnswer' => $attributes['correctAnswer'] );
// wp_interactivity_context( 'create-block', $ourContext );
?>
<div style="background-color: <?php echo esc_attr( $attributes['bgColor'] ); ?>" class="paying-attention-frontend" data-wp-interactive="create-block" <?php echo wp_interactivity_data_wp_context($ourContext) ?>>
