...
class PetAdoptionTablePlugin {
  ...
  function createPet(){
    if(current_user_can('administrator')){
      if(!isset($_POST['incomingpetname']) || empty(trim($_POST['incomingpetname']))){
        wp_safe_redirect(add_query_arg('error', 'emptyname', site_url('/pet-adoption')));
        exit;
      }
      $pet = generatePet();
  ...
  function deletePet(){
    if(current_user_can('administrator')){
      if(!isset($_POST['petid']) || empty(trim($_POST['petid']))){
        wp_safe_redirect(site_url('/pet-adoption'));
        exit;
      }
      $petid = sanitize_text_field($_POST['petid']);

      global $wpdb;
      $wpdb->delete($this->tableName, array('id' => $petid));
      wp_safe_redirect(site_url('/pet-adoption'));
    } else {
      wp_safe_redirect(site_url(''));
    }
  ...
  function onActivate() {
    ...
    dbDelta("CREATE TABLE $this->tableName (
      ...
      birthyear smallint(5) NOT NULL,
      petweight smallint(5) NOT NULL,
